...
# CSS Avançado com Ícone Flutuante
st.markdown("""
<style>
    .main-header {
        font-size: 2.5rem;
    ...
    /* Ícone flutuante do Prof. Tilap-IA */
    .floating-professor {
        position: fixed;
        bottom: 20px;
        right: 20px;
...
def add_consultor_icon():
    """Adiciona ícone do consultor virtual super estilizado"""
        ...
        <div style="text-align: center; margin: 2rem 0;">
            <a href="{PROFESSOR_ASSISTANT_URL}"  style="text-decoration: none;">
                ...
                " onmouseover="
                    ...
                    this.style.boxShadow='0 15px 40px rgba(59, 130, 246, 0.8), 0 0 0 3px rgba(255, 255, 255, 1), 0 0 0 8px rgba(59, 130, 246, 0.4)';
                " onmouseout="
                    this.style.transform='scale(1) rotate(0deg)'; 
                    this.style.boxShadow='0 8px 25px rgba(59, 130, 246, 0.4), 0 0 0 3px rgba(255, 255, 255, 0.8), 0 0 0 6px rgba(59, 130, 246, 0.2)';
                ">
                    <span style="
            ...
            ">
                ...
                <p style="
                    ...
                    font-style: italic;
                ">
                    Seu consultor especializado em aquicultura
                </p>
            </div>
        </div>
...
def main():
    ...
    # Adicionar botão do Prof. Tilap-IA (SINTAXE CORRIGIDA)
    add_consultor_icon()
    ...
    with st.sidebar:
        ...
        # Carregar dados - CHAVE ÚNICA ADICIONADA
        ...
        if st.session_state.sheets_data:
            ...
            if feed_df is not None:
                ...
                # Opção "Todos os tanques"
                all_tanks = st.checkbox("✅ Selecionar todos os tanques", value=True, key="checkbox_todos_tanques_main")
            ...
            confidence_level = st.selectbox(
                ...
                [90, 95, 99],
                index=1,
                help="Nível de confiança para intervalos estatísticos",
            ...
            correction_factor = st.slider(
                ...
                0.5, 2.0, 1.0, 0.1,
                help="Ajuste fino para cálculo do peso estimado",
                key="slider_correcao_main"
            )

            st.markdown('</div>', unsafe_allow_html=True)

            # Botão de processamento - CHAVE ÚNICA ADICIONADA
            if st.button("🚀 Processar Análise Completa", use_container_width=True, key="btn_processar_analise_main"):
...
# CONTINUA NA PARTE 2
# CONTINUAÇÃO DA PARTE 1 - Funções de Cálculo e Análise
...
def calculate_daily_growth(data):
    ...
    # Calcular crescimento diário por tanque
    growth_results = []

    for tanque in data['tanque'].unique():
        tank_data = data[data['tanque'] == tanque].copy()

        # Calcular diferença de peso entre dias consecutivos
...
def perform_advanced_analysis(data, confidence_level):
    ...
    # 2. Regressão linear: Ração vs Peso
    ...
    # 3. Estatísticas descritivas avançadas
    ...
    # 4. Análise de distribuição (teste de normalidade)
...
def calculate_advanced_statistics(series, confidence_level):
    """Calcula estatísticas avançadas com intervalo de confiança"""
    ...
    if len(data) > 1:
        ...
        fig_scatter.add_trace(
            go.Scatter(
                ...
                y=y_trend,
                mode='lines',
                name=f'Tendência (R² = {r2:.3f})',
                line=dict(color='red', width=3, dash='dash')
            )
        )

    fig_scatter.update_layout(
        title="Correlação: Consumo de Ração vs Peso Médio",
...
def display_feed_conversion_analysis(data):
    ...
    **Taxa de Conversão Alimentar (TCA)** = Quantidade de ração fornecida (kg) ÷ (Peso final - Peso inicial) (kg)
    ...
    tca_summary = []
    for tanque in data['tanque'].unique():
        tank_data = data[data['tanque'] == tanque].iloc[0]  # Pegar primeiro registro (TCA é constante)
...
def display_ai_assistant_placeholder():
    ...
    **🚀 Funcionalidade em Desenvolvimento**
    ...
    O Assistente de IA Prof. Tilap-IA está disponível através do ícone flutuante no canto inferior direito da tela.
    ...
    **Como usar:**
    ...
    with col2:
        if st.button("🚀 Acessar Prof. Tilap-IA", use_container_width=True, key="btn_acesso_ia_main"):
            st.markdown("""
            <script>
...
# CONTINUA NA PARTE 3
# CONTINUAÇÃO DA PARTE 2 - Análise Gaussiana e Estatísticas Avançadas
...
def display_gaussian_analysis(data, analysis):
    ...
    # Gráfico da distribuição com curva de Gauss
    fig = go.Figure()
    ...
    if 'normality_test' in analysis:
        ...
        with col1:
            st.markdown('<div class="stat-card">', unsafe_allow_html=True)
            st.metric("📊 Estatística W", f"{normality['shapiro_stat']:.4f}")
            st.markdown('</div>', unsafe_allow_html=True)

        with col2:
            st.markdown('<div class="stat-card">', unsafe_allow_html=True)
            st.metric("📈 p-valor", f"{normality['shapiro_p']:.4f}")
            st.markdown('</div>', unsafe_allow_html=True)
    ...
    st.subheader("📊 Análise por Percentis")
    ...
    perc_df = pd.DataFrame({
        ...
        'Interpretação': [
            'Muito baixo', 'Baixo', 'Abaixo da média', 'Mediana',
            'Acima da média', 'Alto', 'Muito alto'
        ]
    })
...
def display_advanced_statistics(data, analysis):
    ...
    if 'peso_statistics' in analysis:
        ...
        st.write("**🐟 Estatísticas do Peso dos Peixes:**")
        ...
        # Intervalo de confiança
        st.write(f"""
        **Intervalo de Confiança ({peso_stats['confidence_level']}%):** 
        [{peso_stats['ci_lower']:.3f}, {peso_stats['ci_upper']:.3f}] kg
        """)

    # Análise por tanque
    if 'tank_analysis' in analysis:
        st.subheader("🏊 Análise Comparativa por Tanque")
        ...
        for tanque, stats in tank_stats.items():
            tank_comparison.append({
                'Tanque': f'Tanque {tanque}',
                'Peso Médio (kg)': f"{stats['peso_medio']:.3f}",
                'Desvio Padrão': f"{stats['peso_std']:.3f}",
                'Eficiência Média': f"{stats['eficiencia_media']:.2f}",
                'TCA Média': f"{stats['tca_media']:.2f}",
        ...
        fig_comparison = make_subplots(
            rows=1, cols=3,
            subplot_titles=("Peso Médio por Tanque", "Eficiência por Tanque", "TCA por Tanque")
        )
        ...
        fig_comparison.add_trace(
            go.Bar(x=[f'T{t}' for t in tanques], y=eficiencias, name='Eficiência', marker_color='green'),
            row=1, col=2
        )
        ...
        fig_comparison.update_layout(
            title="Comparação de Performance entre Tanques",
            height=400,
            showlegend=False,
            template="plotly_white"
        )
...
def display_welcome_advanced():
    ...
    with col2:
        ...
        #### 🤖 **Assistente de IA**
        - **Prof. Tilap-IA** disponível no ícone flutuante
...
# CONTINUA NA PARTE 4
# CONTINUAÇÃO DA PARTE 3 - Exportação e Funções Finais
...
def display_advanced_export(data, analysis):
    ...
    with col1:
        ...
        export_format = st.selectbox(
            "📁 Formato:",
            ["CSV", "Excel Completo", "PDF Relatório", "JSON Dados"],
            key="selectbox_formato_export_main"
        )

        include_charts = st.checkbox("📊 Incluir gráficos", True, key="checkbox_graficos_main")
        include_statistics = st.checkbox("📈 Incluir estatísticas", True, key="checkbox_estatisticas_main")
        include_analysis = st.checkbox("🔬 Incluir análises", True, key="checkbox_analises_main")
    ...
    with col2:
        ...
        decimal_places = st.slider("🔢 Casas decimais:", 1, 4, 3, key="slider_decimais_main")

        include_metadata = st.checkbox("📝 Incluir metadados", True, key="checkbox_metadata_main")
    ...
    with col1:
        if st.button("📄 Exportar CSV", use_container_width=True, key="btn_export_csv_main"):
    ...
    with col2:
        if st.button("📊 Exportar Excel", use_container_width=True, key="btn_export_excel_main"):
    ...
    with col3:
        if st.button("📋 Exportar PDF", use_container_width=True, key="btn_export_pdf_main"):
    ...
    with col4:
        if st.button("🔗 Exportar JSON", use_container_width=True, key="btn_export_json_main"):
...
def prepare_pdf_export(data, analysis, include_charts):
    ...
    if not PDF_AVAILABLE:
        st.warning("⚠️ Exportação PDF não disponível. Biblioteca ReportLab não instalada.")
        return b""
    ...
    try:
        ...
        story.append(Paragraph("Relatório de Aquicultura Analytics", title_style))
        ...
        resumo_text = f"""
        • Peso médio dos peixes: {peso_medio:.3f} kg<br/>
        • Total de ração consumida: {total_racao:.1f} kg<br/>
        • Eficiência alimentar média: {eficiencia:.2f}<br/>
        • Período analisado: {len(data)} registros<br/>
        • Tanques monitorados: {len(data['tanque'].unique())}
        """
        ...
        if len(data['tanque'].unique()) > 1:
            story.append(Paragraph("Análise por Tanque", styles['Heading2']))

            # Criar tabela
            tank_data = []
            tank_data.append(['Tanque', 'Peso Médio (kg)', 'Eficiência', 'Ração Total (kg)'])
        ...
        if 'tca_periodo' in data.columns:
            ...
            if tca_media < 1.5:
                recomendacao = "Excelente performance. Manter práticas atuais."
            elif tca_media < 2.0:
                recomendacao = "Boa performance. Considerar otimizações pontuais."
            else:
                recomendacao = "Performance pode ser melhorada. Revisar estratégia alimentar."

            story.append(Paragraph(f"TCA Média: {tca_media:.2f} - {recomendacao}", styles['Normal']))

        story.append(Paragraph("• Monitoramento contínuo dos indicadores", styles['Normal']))
        story.append(Paragraph("• Análise semanal de tendências", styles['Normal']))
        story.append(Paragraph("• Controle de parâmetros ambientais", styles['Normal']))
